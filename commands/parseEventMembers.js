...
module.exports = {
    aliases: ["parseEMembers", "pem", "parseE"],
    description: "Checks the list of users in the event voice channel versus the in game image.",
    use: "parseEventMembers [channel number] [image]",
    cooldown: 1,
    type: "mod",
    dms: false,
    public: false,
    ...
    async execute(client, message, settings, guilds, guildMembers) {
        ...
        let channelsDB = await client.models.get("eventraiding").findAll({where: {guildID: message.guild.id}});
        let channels = [];
...
async function replace_letters(value, letterToChange, charToReplaceWith) {
    const permutations = [];
