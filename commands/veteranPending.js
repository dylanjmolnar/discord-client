...
module.exports = {
    aliases: ["vetPending"],
    description: "Lists all pending modules for veteran verification.",
    use: `veteranPending`,
    cooldown: 20,
    type: "mod",
    dms: false,
    ...
    premium: false,
    allChannels: false,
    channels: ["veriVetPendingChannel"],
    roles: [],
    async execute(client, message, settings, guilds, guildMembers) {
        let pendingDB = client.models.get("vetpending");
        let args = message.content.split(" ");
        let thingToDo = args[1];
        let thingToExpel = args.slice(2).join(" ");
        if (thingToDo === "remove") {
            if (!thingToExpel) {
                return message.channel.send(`Please provide a valid ID of the veteran pending person to remove.`);
            } else {
            ...
            let channelID = message.guild.channels.get(settings.veriVetPendingChannel).id;
            let currentFieldNumber = -1;
            let embed = new Discord.MessageEmbed();
            embed.setTitle(`The current pending veteran verifications!`);
